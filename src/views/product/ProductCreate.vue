      ...
      <v-col cols="6">
        ...
        <v-card-text>
          <v-form>
            ...
            <v-row>
              <v-col>``
                <v-btn @click="productCreate()" color="secondary">상품등록</v-btn>
              </v-col>
            </v-row>
          </v-form>
        </v-card-text>
...
export default {
  ...
  methods: {
    async productCreate() {
      try {
        // multi-part-formdata또는 url인코딩 방식인 경우, FormData클래스 사용
        const data=new FormData();
        data.append("name", this.name);
        data.append("category", this.category);
        data.append("price", this.price);
        data.append("stockQuantity", this.stockQuantity);
        // image를 아예 append시킺 않으면, 백센드 dto에 null로 매핑되나, null을 append시키면 백엔드에서 타입에러발생
    ...
    fileUpload(event) {
      // 여러개의 이미지일경우 for문을 실행하여 productImage에 리스트 형태로 push
