  ...
  methods: {
    addCart() {
      ...
      const data = this.productList
          ...
          .map((p) => ({ productId: p.id, productName: p.productName, productCount: p.productCount, price: p.price }));
          data.forEach(p=>store.addCart(p))
    },
    scrollPaging() {
      // 현재 화면 높이 + 스크롤로 이동한 거리 > 전체화면 높이 - n 이 성립하면 추가 데이터 로드
      const isBottom =
    ...
    async loadData() {
      try {
        this.isLoading = true;
        // params라는 이름의 객체를 사용하여 쉽게 파라미터형식으로 url조립
